...
def _make_call(tool="FileHandler", action="list"):
    return MCPCall(tool=tool, action=action, params={"path": "."}, confidence=0.9)


def _make_result(call, success=True):
    return MCPResult(call=call, success=success, output="ok", duration_ms=5.0)


def test_log_creates_file(tmp_path):
    logger = AuditLogger(tmp_path)
    call = _make_call()
    result = _make_result(call)
...
def test_log_entry_has_required_fields(tmp_path):
    logger = AuditLogger(tmp_path)
    call = _make_call()
    logger.log(call, _make_result(call))
    log_file = list(tmp_path.glob("audit_*.jsonl"))[0]
    entry = json.loads(log_file.read_text().strip())
    for field in ("seq", "session_id", "ts", "call", "result", "prev_hash", "entry_hash"):
...
def test_audit_chain_valid(tmp_path):
    logger = AuditLogger(tmp_path)
    for i in range(5):
        call = _make_call(action=f"action_{i}")
        logger.log(call, _make_result(call))
    log_file = list(tmp_path.glob("audit_*.jsonl"))[0]
